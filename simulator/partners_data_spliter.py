...
for chunk in pd.read_csv("D:/data/Criteo_Conversion_Search/CriteoSearchData.csv", sep="\t",
    ...
    for e in partner_id_list:
        if not os.path.isfile("D:/data/saved/"+e+".csv"):
            grouped_chunk.get_group(e).to_csv("D:/data/saved/" + e + ".csv")
        else:
            grouped_chunk.get_group(e).to_csv("D:/data/saved/" + e + ".csv", mode='a', header=False)
    i = i+1
    print(i)
